...
const countFollower = async (followerId) =>{
    const result = await pool.query(
        `
        select Count(*) :: int as count where follower_id = $1 
        `
        [followerId]
    )
...
const countFollowing = async (followingId) =>{
    const result = await pool.query (
        `
        select Count(*)::int as count following_id = $1         
        `
        [followingId]
    )
    return result.rows[0].count
